 ...
 * 1. 模版
 * 基本定义：
 * <{}>标识定义解析串，有五种类型即where,group,having,order,limit,包裹起来的字符串称为域(包含<{}>),比如<{where ...},则称为where域
 * {字段解释:绘制类型}标识定义字段中文解释和绘制类型,不指定则取默认对应表(mysql,可维护)
 * .标识在where域分两种情况：
 * a 表示别名，用于指定相同字段不同含义的key，比如day在新增里表示新增日期,在下载里表示下载日期,可以使用.结合{{}}自定义这两个字段,比如day.1{{新增日期}} day.2{{下载日期}}
 * b 表示引用，结合:使用时，:表示等于,可引用where域中任意key的值,比如model:where.1.md表示model等于第一个where条件中md字段的值
 * 
 * 规则约定：
 * 1.查询最外层必须要有select。废弃： group,having,order,limit域的声明，即<{group by}> ... 
 ...
 * 3.非最外层<{group by...}>只作引用 (废弃) 
 * 4.对于map类型的group by只能作用于单个查询中，不能在嵌套中使用，因为子查询无法解析出map字段
 * 
 * 模版示例：
 ...
 * 参数约定：
 $params = array(
	...
	'where' => array(
	    // 指定key-value
		'fields'   => array(
			'md.1'=>array('huawei p9', 'huawei p10'),
			'br'=>'huawei',
			'ch'=>'100130-100131',
			'os'=>'24',
			'brand' => 'huawei',
			'event_seg' => "map { action : a{equal} , event:222, action:b{like_left}}",
		),
		// 指定运算符
		'operators' => array(
			'md.1'=>'not_equal',
 ...
 * 
 * 3. 解析规则
 * 4. 维表问题：对于某些字段存在相应的维表，但字段和表的字段是对不上的，比如ch对应不上dim_channel2里的channel字段，所以需要配置进行关联。对照表:field_map
 * 
 * 
 * 
 */
